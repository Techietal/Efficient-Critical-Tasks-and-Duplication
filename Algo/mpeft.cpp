...
void perform(vector<vector<int>> &commc,vector<vector<int>> &compc,vector<vector<int>> &task,vector<int> seq,vector<int> &avail,vector<vector<int>> &oct,vector<vector<double>> &k){
        ...
        for(int j = 0;j < avail.size();j++){
            ...
            for(int k = 0;k < seq.size();k++){
                if(commc[k][seq[i] - 1] != -1){
                    if(j == task[k][2]) est = max(est,task[k][1]);
...
int main(){
    ...
    cout << "K values:\n";
    for(int i = 0;i < n;i++){
        for(int j = 0;j < p;j++) cout << kval[i][j] << " ";
        cout << endl;
    }
    cout << "OCT values:\n";
    for(int i = 0;i < n;i++){
        for(int j = 0;j < p;j++) cout << oct[i][j] << " ";
        cout << endl;
    }
    vector<vector<int>> task(n,vector<int> (3));
    vector<int> avail(p,0);
    perform(commc,compc,task,seq,avail,oct,kval);
    cout << "TASKS\tAST\tAFT\tPRO\n";
    for(int i = 0;i < n;i++){
